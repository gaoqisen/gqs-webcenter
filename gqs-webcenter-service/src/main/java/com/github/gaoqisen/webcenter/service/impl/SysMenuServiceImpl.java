...
public class SysMenuServiceImpl extends ServiceImpl<SysMenuMapper, SysMenu> implements SysMenuService {
	...
	@Override
	public List<SysMenu> queryMenuByUserIdAndApplicationNameAndParentId(Integer userId, String applicationName,
			Long parentId) {
		if (userId == null) {
	...
	private List<SysMenu> getMenuTreeList(List<SysMenu> menuList, Integer userId, String applicationName) {
		...
		for (int i = 0; i < menuList.size(); i++) {
			...
			if (sysMenu.getType() == 0) {
				sysMenu.setList(getMenuTreeList(
						queryMenuByUserIdAndApplicationNameAndParentId(userId, applicationName, sysMenu.getMenuId()),
