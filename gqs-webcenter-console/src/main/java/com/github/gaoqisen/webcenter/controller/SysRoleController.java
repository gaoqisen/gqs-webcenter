...
import com.github.gaoqisen.webcenter.entity.*;
import com.github.gaoqisen.webcenter.constant.RedisKeyConstant;

import com.github.gaoqisen.webcenter.service.*;
...
public class SysRoleController {
	...
	public Result update(@RequestBody SysRole sysRole) {
		...
		this.sysRoleMenuService
				.remove(new QueryWrapper<SysRoleMenu>().eq(SysUserRole.COL_ROLE_ID, sysRole.getRoleId()));
		this.sysRoleRestService
				.remove(new QueryWrapper<SysRoleRest>().eq(SysUserRole.COL_ROLE_ID, sysRole.getRoleId()));
		...
		List<SysUserRole> list = this.sysUserRoleService
				.list(new QueryWrapper<SysUserRole>().eq(SysUserRole.COL_ROLE_ID, sysRole.getRoleId()));
		...
		logger.info("[{}]修改成功，角色下所有账号清除登录信息，需重新登录.", sysRole.getRoleName());
		return Result.success();
	}
	...
	public Result info(@PathVariable Long roleId) {
		...
		for (SysCode sysCode : sysCodes) {
			...
			data.put(MENU_CHECK, this.sysRoleMenuService.queryMenuIdByRoleIdAndApplicationName(sysRole.getRoleId(),
					sysCode.getApplicationName()));
			data.put(REST_CHECK, this.sysRestService.queryDigestByApplicationName(sysCode.getApplicationName(),
					sysRole.getRoleId()));
