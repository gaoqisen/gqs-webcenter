...
import com.github.gaoqisen.webcenter.constant.Constant;
import com.github.gaoqisen.webcenter.constant.SysContextConstant;
...
import com.github.gaoqisen.webcenter.form.LoginForm;
...
import com.github.gaoqisen.webcenter.utils.HttpUtil;
...
public class LoginController {
	...
	public Result logout(HttpServletRequest httpRequest) {
		...
		logger.info("[{}]退出系统.", sysUser.getUsername());
		return Result.success();
	}

	@ApiOperation("验证码")
	@GetMapping("captcha.jpg")
	public void captcha(HttpServletRequest httpRequest, HttpServletResponse httpServletResponse, String uuid)
			throws IOException {
		if (StringUtils.isBlank(uuid)) {
			throw new AppException("uuid不能为空");
		}
		String text = producer.createText();
		stringRedisTemplate.opsForValue().set(Constant.CAPTCHA.concat(httpRequest.getSession().getId()), text, 30,
				TimeUnit.MINUTES);
	...
	public Result login(@RequestBody LoginForm loginForm, HttpServletRequest httpServletRequest) {
		...
		String captcha = stringRedisTemplate.opsForValue()
				.get(Constant.CAPTCHA.concat(httpServletRequest.getSession().getId())).toString();
		if (!loginForm.getCaptcha().equalsIgnoreCase(captcha)) {
			return Result.error("验证码输入错误");
		}
		SysUser sysUser = sysUserService
				.getOne(new QueryWrapper<SysUser>().eq(SysUser.COL_USERNAME, loginForm.getUsername()));
		// 账号不存在
		if (sysUser == null) {
		...
		sysUser.setPermissionList(
				sysRestService.getUrlByUserIdAndApplicationName(sysUser.getUserId(), applicationName));
		String redisLoginKey = applicationName.toUpperCase().concat(httpServletRequest.getSession().getId());
		stringRedisTemplate.opsForValue().set(redisLoginKey, JSON.toJSONString(sysUser), 10, TimeUnit.HOURS);
		...
		logger.info("[{}]登录系统.", sysUser.getUsername());
	...
	public void ssoLogin(HttpServletRequest request, HttpServletResponse response, String redirect, String clientId) {
		...
		try {
			if (redisStr == null) {
				String context = SysContextConstant.LOGIN_CONTEXT.replace("CLIENTID", clientId).replace("REDIRECT",
						redirect);
				...
				sysUser.setMenuList(sysMenuService.queryMenuByUserIdAndApplicationName(sysUser.getUserId(),
						sysCode.getApplicationName()));
				sysUser.setPermissionList(sysRestService.getUrlByUserIdAndApplicationName(sysUser.getUserId(),
						sysCode.getApplicationName()));
	...
	public Result ssoLogin(@RequestBody LoginForm loginForm, HttpServletRequest httpServletRequest,
			HttpServletResponse httpServletResponse) {
		// 校验clientId
		SysCode sysCode = sysCodeService
				.getOne(new QueryWrapper<SysCode>().eq(SysCode.COL_CLIENT_ID, loginForm.getClientId()));
		if (sysCode == null) {
		...
		user.setMenuList(
				sysMenuService.queryMenuByUserIdAndApplicationName(user.getUserId(), sysCode.getApplicationName()));
		user.setPermissionList(
				sysRestService.getUrlByUserIdAndApplicationName(user.getUserId(), sysCode.getApplicationName()));
